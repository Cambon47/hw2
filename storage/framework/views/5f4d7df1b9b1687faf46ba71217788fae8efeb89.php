<html >
<head>
    <meta charset="utf-8" />
    <title>Quiz</title>
    
    <link href="https://fonts.googleapis.com/css?family=Pangolin:400,700|Proxima+Nova" rel="stylesheet" type="text/css">
    <link rel="stylesheet" href="css/provided-style.css"/>
    <link rel="stylesheet" href="css/style.css"/>
    <script src="js/constants.js" defer="true"></script>
    <script src="js/script.js" defer="true"></script>
    <meta name="viewport" content="width=device-width, initial-scale=1">
</head>


<body>
<article>
      <header>
          <h1>Web Programming</h1>
          <div>
            <span>M</span>
            <span>H</span>
            <span>W</span>
            <span>2</span>
          </div>
      </header>
      <h1>Dimmi i tuoi luoghi preferiti e ti diro' chi sei</h1>

     <section id="modal-view" class="hidden">
    </section>

      <section class="question-name">
        <h1>Domanda 1: Quale, tra queste citta' francesi, preferisci?</h1>
      </section>

      <section class="choice-grid">
        <div data-choice-id="blep" data-question-id="one">
          <img src="images/annecy.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="happy" data-question-id="one" >
          <img src="./images/Bordeaux.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="sleeping" data-question-id="one">
          <img src="./images/Lione.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="dopey" data-question-id="one">
          <img src="./images/Marsiglia.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="burger" data-question-id="one">
          <img src="./images/Montpellier.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="cart" data-question-id="one">
          <img src="./images/Nantes.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="nerd" data-question-id="one">
          <img src="./images/Nizza.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="shy" data-question-id="one">
          <img src="./images/Parigi.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="sleepy" data-question-id="one">
          <img src="./images/Rennes.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>
      </section>

      <section class="question-name">
        <h1>Domanda 2:Quale, tra queste citta' italiane, preferisci?</h1>
      </section>

      <section class="choice-grid">
        <div data-choice-id="blep" data-question-id="two">
          <img src="images/Bologna.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="happy" data-question-id="two">
          <img src="./images/Catania.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="sleeping" data-question-id="two">
          <img src="./images/Firenze.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="dopey" data-question-id="two">
          <img src="./images/Matera.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="burger" data-question-id="two">
          <img src="./images/Milano.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="cart" data-question-id="two">
          <img src="./images/Rimini.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="nerd" data-question-id="two">
          <img src="./images/Roma.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="shy" data-question-id="two">
          <img src="./images/Torino.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="sleepy" data-question-id="two">
          <img src="./images/Venezia.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>
      </section>

      <section class="question-name">
        <h1>Domanda 3:Quale, tra queste citta' spagnole, preferisci?</h1>
      </section>

      <section class="choice-grid">
        <div data-choice-id="blep" data-question-id="three">
          <img src="images/Barcellona.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="happy" data-question-id="three">
          <img src="./images/Bilbao.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="sleeping" data-question-id="three">
          <img src="./images/Cadice.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="dopey" data-question-id="three">
          <img src="./images/Madrid.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="burger" data-question-id="three">
          <img src="./images/Malaga.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="cart" data-question-id="three">
          <img src="./images/Pamplona.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="nerd" data-question-id="three">
          <img src="./images/Siviglia.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="shy" data-question-id="three">
          <img src="./images/Toledo.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>

        <div data-choice-id="sleepy" data-question-id="three">
          <img src="./images/Valencia.jpg"/>
          <img class="checkbox" src="images/unchecked.png"/>
          <img class="checkbox hidden"  src="images/checked.png"/>
        </div>
      </section>

    </article>

</body>
</html>
<?php /**PATH C:\xampp\htdocs\hw2\resources\views/places.blade.php ENDPATH**/ ?>